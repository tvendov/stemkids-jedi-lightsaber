...
Светещ детски реквизит на STEMKIDS: адресируеми RGB светодиоди с дифузна светлина, звук при включване и движение, поне два външни бутона и безопасно изключване. Това не е лазер, нагревател или предмет за удари. Проектът започва на маса с няколко светодиода и малка сила на звука.

## Какво прави всяка част

Конкретният проект използва FEMTO. Сензорът за движение подава измервания по SoftI2C на P001/P002. Два външни бутона са на P015 / H1.2 и P100 / H1.6, с вътрешен pull-up и контакт към GND. BOOT и RESET не са бутони за играта. Светлината използва директния RGB изход LEDOUT; звукът е отделен път през P014 / DAC, входна мрежа и аудиоусилвател. Няма втори вариант с NANO в тези схеми.
...
Двата бутона минават през независим debounce: първият включва/изключва ефекта, вторият сменя режима. code/femto_buttons.py е polling пример с коментари; задържан при стартиране бутон не включва ефект, преди да бъде отпуснат. Измерването на батерията може да използва ADC и изчислен делител. Файлове, обработка на звук и дълги LED операции не се изпълняват в hard IRQ.

Източник: https://wiki.dfrobot.com/sen0250/

## Аудио и усилватели

За FEMTO предложеният път е P014 / H1.1 / DAC -> DC разделяне и затихване -> DFRobot DFR0119-O / PAM8403 -> говорител. Изходът на микроконтролера не захранва говорител директно. AUDIO_BUTTONS_BG.md и breadboard-controls.png описват отделния монтаж; той още не е изпитан.

DFR0119-O е предложен модул с аналогов вход и 2.54 mm стъпка. Използваме един канал, отделен регулиран 5 V клон с ограничение на тока и обща GND; не подаваме това захранване към FEMTO VBUS/VLED/3V3. Говорителят е между L+ и L-, не към GND. Не съединяваме мостовите изходи или двата канала. SHDN остава несвързан в началния опит; не го свързваме директно към GPIO с 5 V pull-up. Нужен е независим физически ключ на захранването. Започва се с ниска сила на звука.
...
Източник: https://www.dfrobot.com/product-2211.html
...
### REQ-002 · Адресируеми RGB LED през LEDOUT

Статус: Потвърдено · Приоритет: Висок

Директният RGB изход на FEMTO се използва с P112/P500. Бордовият пиксел е индекс 0; външните започват от 1.

Критерий за приемане: Начален 5-пикселен опит с ниска яркост, изгасване при грешка и проверен токов бюджет. Дългото острие има отделно проектирано захранване.
...
### REQ-003 · Ускорение и въртене с DFRobot BMI160

Статус: Потвърдено · Приоритет: Висок

SEN0250 измерва ускорение и ъглова скорост за реактивните ефекти.

Критерий за приемане: Проверени адрес, единици, калибрация, честота и остарели проби. Host моделът не се обявява за готов SmoothSwing.
...
### REQ-004 · Отделен аудиоусилвател

Статус: Потвърдено · Приоритет: Висок

FEMTO DAC P014 подава аналогов сигнал към Class-D усилвател и говорител. Предложен модул: DFRobot DFR0119-O / PAM8403.

Критерий за приемане: Проверени разделяне на DC, входно ниво, управление на звука и захранване. Говорителят е между двата изхода на един канал, не към GND.

Решение: Авторът изисква усилвател; конкретният модул е инженерно предложение, не потвърдена покупка.

История:
- 2026-09-20T00:33:56Z · Потвърдено · STEMKIDS / tvendov · Авторът изисква усилвател; конкретният модул е инженерно предложение, не потвърдена покупка.

### REQ-005 · Поне два отделни физически бутона

Статус: Потвърдено · Приоритет: Висок

Бутон 1 включва/изгасва ефекта. Бутон 2 сменя режим. P015 и P100, активни към GND с pull-up. BOOT/RESET не заместват тези бутони.

Критерий за приемане: Два отделни бутона с debounce; едно натискане поражда едно събитие. Двата входа работят заедно с RGB, IMU и аудио.
...
Сензорът, RGB, двата бутона и аудиопътят са ясно отделени от силовото захранване.

Критерий за приемане: Файловете са достъпни и редактируеми в сайта. Без отбелязан хардуерен PASS преди реален тест.
...
![FEMTO: breadboard връзки за началния опит. Тествано на платка: Не.](schematics/breadboard-controls.png)
...
- [femto\_buttons.py](code/femto_buttons.py)
...
- [test\_femto\_buttons.py](code/test_femto_buttons.py)
- [test\_femto\_rgb.py](code/test_femto_rgb.py)
- [test\_imu\_model.py](code/test_imu_model.py)

## Документи

- [AUDIO\_BUTTONS\_BG.md](documents/AUDIO_BUTTONS_BG.md)
